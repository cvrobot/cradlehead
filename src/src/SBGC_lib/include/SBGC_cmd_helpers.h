...
void SBGC_get_angles_pack(SBGC_get_angles_t &p, SerialCommand &cmd);
inline uint8_t SBGC_get_angles_send(SBGC_get_angles_t &p, SBGC_Parser &parser) {
	SerialCommand cmd;
	SBGC_get_angles_pack(p, cmd);
...
typedef struct {
	uint8_t mode;
	uint8_t slot;
	int8_t reserved[32];  // roll,pitch,yaw
}SBGC_run_script_t;
void SBGC_run_script_pack(SBGC_run_script_t &p, SerialCommand &cmd);
inline uint8_t SBGC_run_script_send(SBGC_run_script_t &p, SBGC_Parser &parser) {
	SerialCommand cmd;
	SBGC_run_script_pack(p, cmd);
	return parser.send_cmd(cmd);
}
